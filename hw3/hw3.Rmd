...
# 2) Plot Residuals
breaks <- seq(0, x_lim, by = binwidth)
...
# observed stat
obs_max <- max(hist(data$location,  breaks = seq(0, x_lim, by = M), plot = F)$counts)
...
# observed stat
obs_second <- sort((hist(data$location,  breaks = seq(0, x_lim, by = M), plot = F)$counts), 
                   decreasing = T)[2]
